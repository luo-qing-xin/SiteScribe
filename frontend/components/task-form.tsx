...
const schema = z.object({ title: z.string().min(1, "请输入待办标题").max(120), description: z.string().min(1, "请输入待办描述").max(5000), assignee_id: z.number().positive("请选择责任人"), due_at: z.string().min(1, "请选择截止时间"), status: z.enum(["OPEN", "IN_PROGRESS", "DONE", "CANCELLED"]) });
...
export function TaskForm({ sourceRecordId, initial, onSaved }: { sourceRecordId?: number; initial?: TaskItem; onSaved: (task: TaskItem) => void }) {
  ...
  const { register, handleSubmit, formState: { errors, isSubmitting } } = useForm<Values>({ resolver: zodResolver(schema), defaultValues: { title: initial?.title ?? "", description: initial?.description ?? "", assignee_id: initial?.assignee.id ?? 0, due_at: localDateTime(initial?.due_at), status: (["OPEN", "IN_PROGRESS", "DONE", "CANCELLED"].includes(initial?.status ?? "") ? initial?.status : "OPEN") as Values["status"] } });
